...
export const dbUpdateAppSettings = async (settingValue, overwrite) => {
  if (overwrite) {
    await appDb.table('pocket_settings').put({ id: 1, ...settingValue });
  } else {
    await appDb.table('pocket_settings').update(1, {
      ...settingValue,
    });
  }
};
...
export const dbGetAppSettings = async () => {
  const congData = await appDb.table('pocket_settings').get({ id: 1 });
  return congData;
};
